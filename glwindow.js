...
class GLWindow {
	...
	setTexture(img) {
		...
		this.#zoom = this.#cvs.width > this.#cvs.height ? this.#cvs.width / this.#texScale.x : this.#cvs.height / this.#texScale.y;
		this.setZoom(this.#zoom);
	}

	setPixelColor(x, y, color) {
		const rgba = new Uint8Array([color[0], color[1], color[2], 255]);
	...
	setZoom(z) {
		this.#zoom = Math.max(0.01, Math.min(z, 40));
		this.#gl.uniform1f(this.#u_zoom, this.#zoom);
	}
	...
	updateViewScale() {
		const w = this.#cvs.clientWidth;
		const h = this.#cvs.clientHeight;
		this.#cvs.width = w;
		this.#cvs.height = h;
	...
	click(pos) {
		...
		if (pos.x < a.x || pos.y < a.y || pos.x > b.x || pos.y > b.y) return;
